"""
Atomic token storage for TIDAL user credentials.

Stores OAuth tokens (access_token, refresh_token, etc.) in a JSON file.
Writes are atomic (temp file + rename) so a crash mid-write never corrupts.

Storage locations (first writable wins):
  1. /etc/beosound5c/tidal_tokens.json  (production on Pi)
  2. <script_dir>/tidal_tokens.json      (dev fallback)
"""
...
STORE_PATHS = [
    ...
    os.path.join(SCRIPT_DIR, "tidal_tokens.json"),
]
