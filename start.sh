...
nohup python3 -m http.server 8000 > "$LOG_DIR/http.log" 2>&1 &

echo "🔧 Starting server.py..."
cd "$BASE_DIR/hw" || exit 1
nohup sudo python3 server.py > "$LOG_DIR/server.log" 2>&1 &

echo "🛰️  Starting sniffer.py..."
nohup sudo python3 sniffer.py > "$LOG_DIR/sniffer.log" 2>&1 &
...
cd "$BASE_DIR/web" || exit 1
nohup ./web.sh > "$LOG_DIR/websh.log" 2>&1 &
